<script lang="ts">
  ...
  function computeParameters() {
    ...
    const specificFieldValuePart = meetingFieldValue ?
      '&meeting_key=' + encodeURIComponent(keyForMeetingKeyValue) + '&meeting_key_value=' + encodeURIComponent(meetingFieldValue)
      : '';

    parameters = usedFormatsPart + onWeekdayPart + notOnWeekdayPart + hasVenueTypePart + doesNotHaveVenueTypePart
      + hasFormatPart + doesNotHaveFormatPart + formatsComparisonOperatorPart + specificFieldValuePart;
  }
    ...
    <TableBody>
      ...
      <TableBodyRow>
        <TableBodyCell>
          ...
          <Select id="select-value" class="mt-2" items={meetingKeyValues} placeholder={$translations.chooseOption}
            disabled={keyForMeetingKeyValue === ''}
            bind:value={meetingFieldValue} onchange={computeParameters} />
        </TableBodyCell>
        <TableBodyCell>
          <Label for="enter-new-value" class="mb-2">{$translations.enterNewValue}:</Label>
          <Input type="text" id="enter-new-value" disabled={keyForMeetingKeyValue === ''}
            placeholder="" bind:value={meetingFieldValue} on:input={computeParameters} />
        </TableBodyCell>
      </TableBodyRow>
    </TableBody>
  </Table>
